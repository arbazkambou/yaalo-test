  ...
  return totalCartItems.length !== 0 ? (
    <>
      ...
      <div className="border-t px-4 pb-3 pt-3 sm:px-6">
        <ApplyCouponCode buttonStyle="h-max" />

        {/* Subtotal */}
        <div className="mb-2 flex items-center justify-between">
          <p className="font-montserrat text-lg font-semibold">{t("subTotal")}</p>
          <span className="flex items-center justify-center rounded-[6px] bg-primary/15 px-[12px] py-[4px] text-body-base font-medium text-foreground">
        ...
        <div className="flex flex-col gap-1.5">
          ...
          >
            <PrimaryButton
              className="bg-primary hover:text-foreground hover:bg-foreground hover:text-background text-foreground w-full"
              role="link"
            >
              {t("checkout")}{" "}
              {isCheckoutSummaryLoading ? (
          ...
          <Button
            variant={"ghost"}
            size={"sm"}
            className="text-body-base hover:text-primary border-dashed!"
            onClick={() => {
