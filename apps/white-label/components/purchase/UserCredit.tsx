...
function UserCredit() {
  ...
  if (isLoading) {
    return <Skeleton className="h-[1.1885rem] w-[70px] bg-accent" />;
  }
  ...
  return (
    <div className="flex w-full items-center justify-center gap-2 rounded-[0.5rem] bg-muted px-[1rem] py-[0.89rem] xl:w-max">
      ...
      <p className="text-body-md font-500">{t("currentBalance")}</p>

      <p className="text-body-md font-600 text-primary">${user?.balance}</p>
    </div>
  );
}
