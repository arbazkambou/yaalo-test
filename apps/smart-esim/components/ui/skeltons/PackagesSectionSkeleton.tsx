export default function PackagesSectionSkeleton() {
  return (
    <section className="container mt-16 animate-pulse">
      {/* MOBILE LAYOUT */}
      <div className="lg:hidden">
        {/* Header */}
        <div className="flex items-center justify-center gap-2 mb-6">
          <div className="h-4 w-4 rounded bg-gray-200" />
          <div className="h-4 w-32 rounded bg-gray-200" />
        </div>

        {/* Title block */}
        <div className="flex flex-col items-center text-center gap-4 mb-6">
          <div className="h-12 w-12 rounded-full bg-gray-200" />
          <div className="h-8 w-56 rounded bg-gray-200" />
          <div className="space-y-2 w-full max-w-sm">
            <div className="h-4 w-full rounded bg-gray-200" />
            <div className="h-4 w-11/12 rounded bg-gray-200" />
            <div className="h-4 w-10/12 rounded bg-gray-200" />
          </div>
        </div>

        {/* Action buttons */}
        <div className="flex items-center justify-between gap-3 mb-4">
          <div className="h-10 flex-1 rounded-full bg-gray-200" />
          <div className="h-10 w-24 rounded-full bg-gray-200" />
        </div>

        {/* List area */}
        <div className="overflow-hidden rounded-2xl border border-gray-200">
          <div className="max-h-[520px] overflow-y-auto">
            <PackageRowSkeletonMobile active />
            <PackageRowSkeletonMobile />
            <PackageRowSkeletonMobile />
            <PackageRowSkeletonMobile />
          </div>
        </div>
      </div>

      {/* DESKTOP LAYOUT */}
      <div className="hidden lg:grid gap-8 lg:grid-cols-[500px_1fr]">
        {/* LEFT HERO CARD */}
        <div className="relative overflow-hidden rounded-[48px] bg-gray-200">
          <div className="h-[720px] w-full" />
          <div className="absolute inset-0 flex flex-col items-center justify-start pt-10 px-6">
            <div className="h-4 w-32 rounded bg-gray-300" />
            <div className="mt-6 h-10 w-56 rounded bg-gray-300" />
            <div className="mt-6 space-y-3 w-full max-w-sm">
              <div className="h-4 w-full rounded bg-gray-300" />
              <div className="h-4 w-11/12 rounded bg-gray-300" />
              <div className="h-4 w-10/12 rounded bg-gray-300" />
            </div>
          </div>
        </div>

        {/* RIGHT SIDE */}
        <div className="flex flex-col">
          <div className="mb-4 flex items-center justify-end gap-3">
            <div className="h-10 w-44 rounded-full bg-gray-200" />
            <div className="h-10 w-24 rounded-full bg-gray-200" />
          </div>

          <div className="flex-1 overflow-hidden rounded-2xl border border-gray-200">
            <div className="max-h-[620px] overflow-y-auto">
              <PackageRowSkeletonDesktop active />
              <PackageRowSkeletonDesktop />
              <PackageRowSkeletonDesktop />
              <PackageRowSkeletonDesktop />
            </div>

            <div className="border-t border-gray-200 bg-white p-4">
              <div className="grid gap-4 md:grid-cols-[200px_1fr_1fr]">
                <div className="h-14 rounded-xl bg-gray-200" />
                <div className="h-14 rounded-xl bg-gray-200" />
                <div className="h-14 rounded-xl bg-gray-200" />
              </div>
            </div>
          </div>
        </div>
      </div>
    </section>
  );
}

function PackageRowSkeletonMobile({ active }: { active?: boolean }) {
  return (
    <div
      className={[
        "border-b border-gray-200 p-4",
        active ? "bg-gray-100" : "bg-white",
      ].join(" ")}
    >
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          <div className="h-5 w-44 rounded bg-gray-200" />
          <div className="mt-3 h-8 w-24 rounded-full bg-gray-200" />
          <div className="mt-3 h-3 w-16 rounded bg-gray-200" />
        </div>

        <div className="flex flex-col items-end gap-3">
          <div className="h-6 w-16 rounded bg-gray-200" />
          <div className="h-6 w-6 rounded-full bg-gray-200" />
        </div>
      </div>
    </div>
  );
}

function PackageRowSkeletonDesktop({ active }: { active?: boolean }) {
  return (
    <div
      className={[
        "border-b border-gray-200 p-5",
        active ? "bg-gray-100" : "bg-white",
      ].join(" ")}
    >
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0 flex-1">
          <div className="h-5 w-56 rounded bg-gray-200" />
          <div className="mt-3 h-8 w-24 rounded-full bg-gray-200" />
        </div>

        <div className="flex items-center gap-4">
          <div className="h-6 w-16 rounded bg-gray-200" />
          <div className="h-6 w-6 rounded-full bg-gray-200" />
        </div>
      </div>

      <div className="mt-4 h-8 w-24 rounded bg-gray-200" />
    </div>
  );
}
