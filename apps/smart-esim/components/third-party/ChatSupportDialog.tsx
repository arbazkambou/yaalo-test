...
import { ChatSettings } from "@workspace/core/types/services/support/support.types";
...
export function ChatSupportDialog({ whatsappNumber, chat }: PropsType) {
  const { propertyId, widgetId } = chat.tawk;
  ...
  return (
    <div className="fixed bottom-[90px] right-5 z-50 xl:bottom-[80px]">
      ...
      >
        ...
        {/* Live Chat Button */}
        <Link
          href={`https://tawk.to/chat/${widgetId}/${propertyId}`}
          target="_blank"
          rel="noopener noreferrer"
          onClick={() => setIsOpen(false)} // optional: close after clicking
